...
namespace YP.ZReg.Repositories.Implementations
{
    public class DeudaRepository(IBaseRepository _bre) : IDeudaRepository
    {
        ...
        public async Task InsertarDeuda(Deuda deuda)
        {
            try
            {
                List<SqlParameter> parameters = [
                    ...
                    new(){ ParameterName = "@p_importe_total", DbType = DbType.Decimal , Value = deuda.importe_total},
        ...
        public async Task<List<Cliente>> ConsultarDeuda(string empresa, string servicio, string llave, CancellationToken ct = default)
        {
            ...
            var response = await bre.EjecutarConsultaJoinSpAsync<Cliente, Deuda, long>(
